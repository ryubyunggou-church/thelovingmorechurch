...
## 8. 진입점(EditableBlock) 트리거 UX — 2026-08-21 갱신

기존에는 우상단에 작은 "✏ 편집" 버튼만 보여 편집 가능 영역이라는 인지가 약했다. 다음 표준으로 개선했다.

1. **영역 강조** — 호버(데스크톱) / 탭(모바일) 시 블록 전체(`children`)를 감싸는 골드(`border-gold`) 2px 테두리 표시.
2. **안내 오버레이** — 같은 트리거에 반투명 라운드 박스(`bg-paper/70`, `rounded-[20px]`)가 겹쳐지고 중앙에 `'편집'을 클릭하시면 내용을 수정할 수 있습니다.` 안내문 표시. 클릭은 통과시키지 않도록 `pointer-events-none`.
3. **트리거와 버튼 연결** — 오버레이와 "편집" 버튼이 같은 우상단 모서리에서 겹쳐 하나의 도형처럼 보이도록 배치(버튼은 `z-10`, 오버레이는 `z-[5]`).
4. **표시 시점**
   - 데스크톱: `sm:group-hover:opacity-100` — 마우스를 올렸을 때만.
   - 모바일: 편집 버튼은 항상 노출(`opacity-100`), 오버레이는 카드를 탭해야 노출(로컬 `revealed` state 토글). 모달을 닫으면 `revealed`는 자동으로 초기화.

구현: `src/components/shared/EditableBlock.tsx`. 공용 컴포넌트라 이 문서 §4의 대상 모달 전부(11개 화면)에 동일 적용됨 — 화면별 추가 작업 불필요.
